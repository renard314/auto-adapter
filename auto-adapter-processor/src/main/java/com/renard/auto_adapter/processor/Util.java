...
public final class Util {
     ...
     *
     * @param  innerClassSeparator  either '.' or '$', which will appear in a class name like "java.lang.Map.Entry" or
     *                              "java.lang.Map$Entry". Use '.' for references to existing types in code. Use '$' to
     *                              define new class names and for strings that will be used by runtime reflection.
     */
    private static void typeToString(final TypeMirror type, final StringBuilder result,
            final char innerClassSeparator) {
        type.accept(new SimpleTypeVisitor6<Void, Void>() {
                @Override
                public Void visitDeclared(final DeclaredType declaredType, final Void v) {
                ...
                @Override
                public Void visitTypeVariable(final TypeVariable typeVariable, final Void v) {
                    result.append(typeVariable.asElement().getSimpleName());
                    return null;
                }

                @Override
                public Void visitError(final ErrorType errorType, final Void v) {
                ...
                @Override
                protected Void defaultAction(final TypeMirror typeMirror, final Void v) {
    ...
    private static void rawTypeToString(final StringBuilder result, final TypeElement type,
            final char innerClassSeparator) {
    ...
    /**
     * An exception thrown when a type is not extant (returns as an error type), usually as a result of another
     * processor not having yet generated its types upon which a dagger-annotated type depends.
     */
    private static final class CodeGenerationIncompleteException extends IllegalStateException {
        CodeGenerationIncompleteException(final String s) {
            super(s);
        }
    }
}
